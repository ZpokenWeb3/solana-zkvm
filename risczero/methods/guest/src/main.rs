...
fn main() {
    ...
    let input: Vec<u8> = env::read();
    let emulator = SolanaSimulator::default();
    let tx: VersionedTransaction = bincode::deserialize(&input).unwrap();
    let sanitized = emulator.sanitize_transaction(tx, false).unwrap();
    let hash: Hash = Hash::new(&[1; 32]);
    println!("Result: {:?}", emulator.process_transaction(hash, &sanitized).unwrap().result);
